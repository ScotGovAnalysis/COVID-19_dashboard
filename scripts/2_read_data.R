...
pwalk(datasets[["sg_template"]][["TEXT"]] %>%
        filter(worksheet_name %in% names(download_data)) %>%
        select(position, worksheet_name, TITLE_max_35_characters, source, methodology), #..1 is position, ..2 worksheet_name, and so on
      ~ {
        ...
        addWorksheet(download_wb, sheetName = ..2)
        setColWidths(download_wb, ..2, width = c(10, rep("auto", ncols - 1)), cols = 1:ncols, ignoreMergedCells = TRUE)
        ...
        #add formatting
        addStyle(download_wb, ..2, rows = 1, cols = 1, style = createStyle(fontSize = 14, textDecoration = "bold"))
        addStyle(download_wb, ..2, rows = 4, cols = 1:ncols, style = createStyle(textDecoration = "bold", border = c("top", "bottom")))
        addStyle(download_wb, ..2, rows = nrows + 5, cols = 1:ncols, style = createStyle(border = c("top")))
        pwalk(list(data_table, names(data_table), seq_along(data_table)),
              function(x, name, id, sheet = ..2) {
        ...
        #write spreadsheet
        writeData(download_wb, ..2, x = ..3) #title
        writeData(download_wb, ..2, x = link, startRow = 2) #source/link
        writeData(download_wb, ..2, x = data_table, startRow = 4) #table
        writeData(download_wb, ..2, x = "Notes:", startRow = nrows + 6)
        writeData(download_wb, ..2, x =  ..5, startRow = nrows + 7) #methodology
      })
